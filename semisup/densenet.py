...
class DenseNet:
    def __init__(self, data_shape, n_classes, growth_rate=12, depth=40,
                 total_blocks=3, keep_prob=0.8,
                 weight_decay=1e-4, nesterov_momentum=0.9, model_type='DenseNet-BC', dataset='data',
                 should_save_logs=False, should_save_model=False, is_training=True,
                 renew_logs=False,
                 reduction=0.5,
                 bc_mode=False,
                 **kwargs):
        ...
        self.is_training = is_training  # tf.placeholder(tf.bool, shape=[])
    ...
    def transition_layer_to_features(self, _input):
        """This is last transition to get flat features. It perform:
        - batch normalization
        - ReLU nonlinearity
        - wide average pooling
        """
    ...
    def dropout(self, _input):
        if self.keep_prob < 1 and self.is_training:
    ...
    def build_embeddings(self, input):
        ...
        with tf.variable_scope("Transition_to_features"):
            output = self.transition_layer_to_features(output)
    ...
    def _build_graph(self):
        logits = self.build_embeddings(self.images)
